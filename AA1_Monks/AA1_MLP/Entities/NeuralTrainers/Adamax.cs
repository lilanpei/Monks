...
namespace AA1_MLP.Entities.NeuralTrainers
{
        ...
        private static Dictionary<int, Matrix<double>> PerformBatchComputations(AdamParams passedParams, Matrix<double> batchesIndices, int adamUpdateStep, ref double iterationLoss, Dictionary<int, Matrix<double>> firstMoment, Dictionary<int, Matrix<double>> ut, Dictionary<int, Matrix<double>> mhat, Dictionary<int, Matrix<double>> vhat, Dictionary<int, Matrix<double>> prevFirstMoment, Dictionary<int, Matrix<double>> previousUT, int batchIndex)
        {
            Dictionary<int, Matrix<double>> previousWeightsUpdate;
            {
                ...
                foreach (int k in batchIndices)//for each elemnt in th batch
                    batchLoss = PerformExampleComputations(passedParams, firstMoment, ut, mhat, vhat, prevFirstMoment, previousUT, batchIndex, batchLoss, weightsUpdates, k);//per example in the batch
                ...
                for (int y = 0; y < weightsUpdates.Keys.Count; y++)
                {
                    ...
                    firstMoment[y] = passedParams.beta1 * prevFirstMoment[y] + (1 - passedParams.beta1) * (-1 * weightsUpdates[y]);
                    ut[y] = CreateMatrix.Dense(weightsUpdates[y].RowCount, weightsUpdates[y].ColumnCount, 0.0);
                    for (int i = 0; i < ut[y].RowCount; i++)
                    {
                        for (int j = 0; j < ut[y].ColumnCount; j++)
                        {
                            ut[y][i, j] = Math.Max((passedParams.beta2 * previousUT[y])[i, j], weightsUpdates[y].PointwiseAbs()[i, j]);
            ...
            Dictionary<int, Matrix<double>> previousWeightsUpdate;
            {
                ...
                for (int layerIndex = passedParams.network.Layers.Count - 1; layerIndex >= 1; layerIndex--)
                {
                    ...
                    if (!firstMoment.ContainsKey(layerIndex - 1))
                    {
                        ...
                        ut.Add(layerIndex - 1, CreateMatrix.Dense(passedParams.network.Weights[layerIndex - 1].RowCount, passedParams.network.Weights[layerIndex - 1].ColumnCount, 0.0));
                        ...
                        previousUT.Add(layerIndex - 1, CreateMatrix.Dense(passedParams.network.Weights[layerIndex - 1].RowCount, passedParams.network.Weights[layerIndex - 1].ColumnCount, 0.0));
