...
namespace AA1_MLP.Entities.RegressionTrainers
{
    public class LLSNormal : IOptimizer
    {
        ...
        public override List<double[]> Train(TrainersParams.TrainerParams trainParams)
        {
            ...
            var x = trainParams.trainingSet.Inputs.PseudoInverse().Multiply(trainParams.trainingSet.Labels);

            var cost = CostFunction(trainParams.trainingSet.Inputs, trainParams.trainingSet.Labels, x);
            var valCost = CostFunction(trainParams.validationSet.Inputs, trainParams.validationSet.Labels, x);
