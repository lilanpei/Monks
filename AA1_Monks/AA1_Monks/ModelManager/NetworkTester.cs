        ...
        public static void TestNetwork(string testSetLocation, BasicNetwork network, string errorLogLocation)
        {
                ...
                new System.IO.StreamWriter(errorLogLocation))
            {
                ...
                foreach (var pair in testingSet)
                {
                    ...
                    if ((o[0] >= 0.5 ? 1 : 0) == 1 && (int)pair.Ideal[0] == 1)
                    {
                        TP++;
                    }
                    else if ((o[0] >= 0.5 ? 1 : 0) == 0 && (int)pair.Ideal[0] == 0)
                    {
                        TN++;
                    }

                    else if ((o[0] >= 0.5 ? 1 : 0) == 1 && (int)pair.Ideal[0] == 0)
                    {
                        FP++;
                    }
                    else if ((o[0] >= 0.5 ? 1 : 0) == 0 && (int)pair.Ideal[0] == 1)
                    {
                        FN++;
                    }
                    file.WriteLine("Actual=" + (o[0] >= 0.5 ? 1 : 0) + ", Ideal=" + pair.Ideal[0]);
